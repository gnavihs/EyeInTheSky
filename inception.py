...
if __name__ == '__main__':
    ...
    pred = model.classify(image_path=image_path[0])

    # Print the scores and names for the top-10 predictions.
    model.print_scores(pred=pred, k=10)

    file_path_cache_train = os.path.join(cache_data_path, 'inception_train.pkl')
    # file_path_cache_test = os.path.join(data_path, 'inception_cifar10_test.pkl')
    transfer_values = transfer_values_cache(cache_path=file_path_cache_train, model=model, image_paths=image_path)
    print("Cache saved. Remember to delete cache if images are updated")
