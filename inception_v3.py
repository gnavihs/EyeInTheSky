...
def InceptionV3(include_top=False,
                ...
                input_shape=None,
                pooling=None,
                trainable=True, # flag used for making all the layers trainable or not
                classes=1000,
                second_stage=False, # flag used for identifying stage
                model_weights=''):
    ...
    # Load weights
    if weights == 'imagenet' and second_stage == False:
        if K.image_data_format() == 'channels_first':
    ...
    model = Model(img_input, x_newfc, name='my_inception_v3')

    if weights == 'imagenet' and second_stage == True:
        model.load_weights(model_weights)


    # Toggle the trainability of layers
